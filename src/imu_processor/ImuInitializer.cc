 ...
 *    外参R标定
 * 参数:
 *    all_laser_transforms :  pair<time,laser_transform> {pre_integration,transform}
...
//    cout << (transform_lb.rot.template cast<double>() * delta_qij_imu).coeffs().transpose() << endl;
  ...
  //0614:svd to get extrinsic rotation R : transform_lb;
  Eigen::JacobiSVD<MatrixXd> svd(A, Eigen::ComputeFullU | Eigen::ComputeFullV);
  Eigen::Matrix<double, 4, 1> x = svd.matrixV().col(3);
  Quaterniond estimated_qlb(x);
  transform_lb.rot = estimated_qlb.cast<float>().toRotationMatrix(); //normalized?????????????????????????????????????????????????????????????
/*
 * 隐式转换
  Quaterniond a;
  Eigen::Matrix3d R111111=Eigen::Matrix3d::Identity();
  a = R111111;
 */
//   Quaterniond a={1,0,0,0};
//   Eigen::Matrix3d R111111;
//   R111111 = a;
  Vector3d cov = svd.singularValues().tail<3>();
//  DLOG(INFO) << "x: " << x.transpose();

  LOG(INFO) << "extrinsic rotation: " << transform_lb.rot.coeffs().transpose();
    /***
   * add a verify!!!!!!!
   ***/
//  Eigen::Matrix3d XXX = static_cast<float>(transform_lb.rot);
  //Eigen::Matrix3d XXX = (transform_lb.rot).cast<double>().toRotationMatrix();
...
//   LOG(INFO) << "extrinsic rotation2: " << "                  "<< tran2.rotation();
  ///******************************************************************************////
 // Eigen::Quaterniond q = tran.rotate;
 // LOG(INFO) << "extrinsic rotation: " << q.coeffs().transpose();
 ...
 *    g                    : gravity acc0_
 *    transform_lb         : R
 *    R_WI                 : rotation,   convert imu to lidar world
 * 步骤：
 *    （1）陀螺仪bias修正；
 *    （2）初始化速度、重力向量等；
 *    （3）修正重力加速度；
