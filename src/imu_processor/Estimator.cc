 ...
 *    imu信息处理装填
 *  参数：
 *    dt---------------------两次IMU测量的时间间隔//void Estimator::ProcessEstimation()
 ...
 *  参数：
 *     transform_in--------和init的坐标变换
 *     header--------------时间戳等
 *        在void Estimator::ProcessCompactData中调用ProcessLaserOdom(transform_to_init_, header);
 *  其他参数：
 *     laser_transform : struct LaserTransform, {time,transform,pre_integration}
 *     pre_integrations_: window_size, save pre_integration;
 *     all_laser_transforms: pair<header,LaserTransform> map;
 *     transform_lb :  class Transform, {Quaternion,translation}, base to lidar ?  calibration R;
 *           estimate.h --- Transform transform_lb_{Eigen::Quaternionf(1, 0, 0, 0), Eigen::Vector3f(0, 0, -0.1)}; ///< Base to laser transform
 *     Q_WI_    : Quaternion, convert imu to lidar world
 *     R_WI_    : rotation,   convert imu to lidar world
 *     Rs_     :  imu rotation matrix
 *     Ps_     :  imu position
*/
void Estimator::ProcessLaserOdom(const Transform &transform_in, const std_msgs::Header &header) {
  ...
  if (stage_flag_ != INITED  && laser_odom_recv_count_ % estimator_config_.init_window_factor != 0) { 
    return;// better for initialization
  }
  ...
  // TODO: avoid memory allocation?    特征点等？
    ...
    switch (stage_flag_) {
      case NOT_INITED: {
        ...
        if (cir_buf_count_ == estimator_config_.window_size) {
          tic_toc_.Tic();//记录时间
	  //0614 :imu.factor is set "true" ....  imu need calibration???
          //0614 :if no need;
	  ...
	  {
            //0614:  system initialization part, when extrinsic_stage_==2 , calibration is inneed;
            if (extrinsic_stage_ == 2) 
	    {
              // TODO: move before initialization
	      //0614: start estimation extrinsic R , and return the result successful or fail;
              ...
              LOG(INFO) << ">>>>>>> extrinsic calibration"
                        << (extrinsic_result ? " successful"
                                             : " failed")
                        << " <<<<<<<";
	      //ROS_INFO << "extrinsic rotation: " << transform_lb_.rot.coeffs().transpose();
              if (extrinsic_result) 
	      {
                extrinsic_stage_ = 1;
            ...
            //0614: if system finished estimate R , run ininitializaiton and return the result successful or fail;
	  ...
	  //0614: if the ininitializaiton result is successful
          if (init_result) 
	  {
            stage_flag_ = INITED;
            SetInitFlag(true);
	    ...
	    //0614:turn off the odom , no longer used to deskew??
            ...
            for (size_t i = 0; i < estimator_config_.window_size + 1; ++i) 
	    {
          ...
          else  //对应if (init_result)
	  {
            SlideWindow();
          }
        }
        else  //对应if (cir_buf_count_ == estimator_config_.window_size) 
	{
...
//  g_vec_ = Eigen::Vector3d(0.0, 0.0, -1.0) * g_norm_;
      ...
      /*
       * finish ininitializaiton, some rotation set????
       */
  // TODO: update states Ps_
  for (size_t i = 0; i < estimator_config_.window_size + 1;
       ++i) {
